...
import { getRoleDetail, updateRole } from "../../../../services/roleApi";
...
export default function EditRoleScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error(err);
        Alert.alert("Error", "Couldn't load role details.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(err);
      const message = err?.response?.data?.detail || "Couldn't update role.";
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="flex-row items-center justify-between px-4 pt-14 pb-4 bg-white border-b border-gray-200">
        <View className="flex-row items-center">
          ...
          <Text className="text-lg font-semibold text-gray-900 ml-3">Edit Role</Text>
        </View>
        <TouchableOpacity
          onPress={handleSave}
          disabled={saving}
