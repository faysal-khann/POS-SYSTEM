...
export default function UserRow({
  ...
  const roleColor = ROLE_COLORS[user.RoleName] ?? { bg: "bg-gray-100", text: "text-gray-600" };
    ...
    <View className="bg-white border border-gray-200 rounded-xl p-4 mb-3">
      ...
      <View className="flex-row justify-between items-center mb-3">
        <View className={`px-2.5 py-1 rounded-full ${roleColor.bg}`}>
          <Text className={`text-xs font-medium ${roleColor.text}`}>{user.RoleName}</Text>
        </View>
      ...
      <View className="flex-row justify-end border-t border-gray-100 pt-2">
        <TouchableOpacity
        //   onPress={() => router.push(`/(tabs)/users-roles/users/view/${user.UserID}`)}
          className="mr-4"
        >
          <Ionicons name="eye-outline" size={18} color="#3B82F6" />
        </TouchableOpacity>
        <TouchableOpacity
        //   onPress={() => router.push(`/(tabs)/users-roles/users/${user.UserID}`)}
          className="mr-4"
        >
